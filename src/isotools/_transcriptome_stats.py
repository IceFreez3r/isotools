...
def alternative_splicing_events(self, min_total=100, min_alt_fraction=.1, samples=None, region=None, query=None, progress_bar=False):
    ...
    for gene in self.iter_genes(region=region, query=query, progress_bar=progress_bar):
...
def filter_stats(self, tags=None, groups=None, weight_by_coverage=True, min_coverage=2, tr_filter={}):
    ...
    for gene, transcript_id, transcript in self.iter_transcripts(**tr_filter):
        ...
        relevant_filter = [tag for tag in tags if filterfun[tag](g=gene, transcript_id=transcript_id, **transcript)]
