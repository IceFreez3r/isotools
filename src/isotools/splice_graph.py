...
class SpliceGraph():
    ...
    def from_transcript_list(cls, transcripts, strand):
        ...
        graph = SortedDict()
        fwd_starts = [(transcript[0][0], True) for transcript in transcripts]  # genomic 5'
        rev_starts = [(transcript[-1][1], False) for transcript in transcripts]  # genomic 3'

        for transcript_number, transcript in enumerate(transcripts):
            graph.setdefault((transcript[0][0], True), ({}, {}))

            for i, (b1, b2) in enumerate(pairwise(pos for exon in transcript for pos in exon)):
    ...
    @experimental
    def add(self, transcript) -> None:
        '''Add one transcript to the existing graph.

        :param transcript: A list of exon tuples representing the transcript to add
        :type transcript: list'''

        transcript_umber = len(self._fwd_starts)
        self._fwd_starts.append((transcript[0][0], True))  # genomic 5'
        self._rev_starts.append((transcript[-1][1], False))  # genomic 3'
        self._graph.setdefault((transcript[0][0], True), ({}, {}))
        for i, (b1, b2) in enumerate(pairwise(pos for exon in transcript for pos in exon)):
            self._graph.setdefault((b2, bool(i % 2)), ({}, {}))
            self._graph[b2, bool(i % 2)][1][transcript_umber] = b1, not bool((i) % 2)  # successor
            self._graph[b1, not bool(i % 2)][0][transcript_umber] = b2, bool((1) % 2)
